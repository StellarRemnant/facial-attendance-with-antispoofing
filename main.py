import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), 'a_s'))
import pickle
import cvzone
import numpy as np
import cv2
    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                captured_image = img[y1:y2, x1:x2]
            
                result = test(captured_image, "a_s/resources/anti_spoof_models", device_id=0)

                if result != 1:
                    modeType = 4  # Spoof detected
                    counter = 0
                    imgBackground[44:44 + 633, 808:808 + 414] = imgModeList[modeType] 
                    cv2.imshow("Face Attendance", imgBackground) 
                    cv2.waitKey(1000)  # Wait 1 seconds while showing the spoof screen
                    continue  
        ...
        if counter != 0:
            ...
            if modeType != 3:
                if counter <= 10:
                    cv2.putText(imgBackground, str(student_info['total_attendance']), (861, 125), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1)

                    cv2.putText(imgBackground, str(student_info['major']), (1006, 550), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)

                    cv2.putText(imgBackground, str(id), (1006, 493), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)

                    cv2.putText(imgBackground, str(student_info['year']), (1025, 625), cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)

                    cv2.putText(imgBackground, str(student_info['starting_year']), (1125, 625), cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
